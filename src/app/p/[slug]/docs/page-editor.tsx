...
export default function PageEditor({ page, projectId, onDone, onTitleSaved, focusTitle = false, metadata }: { page: DocPage; projectId: string; onDone: () => void; onTitleSaved: (title: string) => void; focusTitle?: boolean; metadata: React.ReactNode }) {
  ...
  const save = useCallback(async (): Promise<boolean> => {
    ...
    if (snapshotDraft() === saved.current) return true;
    ...
    flight.current = (async () => {
      try {
        const response = await fetch(`/api/doc-pages/${page.id}`, { method: 'PATCH', headers: { 'content-type': 'application/json' },
          body: JSON.stringify({ ...JSON.parse(snapshot), updatedAt: stamp.current,forceRevision:forceRevision.current }) });
  ...
  useEffect(() => {
    if (stopped.current || snapshotDraft() === saved.current) return;
  ...
  async function done() {
    ...
    // A new keystroke while a request is in flight needs its own save.
    if (!await save()) return;
    if (snapshotDraft() !== saved.current && !await save()) return;
    if (snapshotDraft() === saved.current) onDone();
  }
  ...
  return <div className="page-editor">
    ...
    <div className="editor-savebar"><span role="status">{status}</span>
      <button type="button" disabled={uploads > 0} onClick={() => void done()}><DocIcon name="check" />Done editing</button>
    ...
    {error && <div className="editor-error" role="alert"><p>{error}</p>
    ...
    {pageSections(page.template).map(([key, label], index) => <section key={key} className="doc-section">
      ...
      <RichEditor key={`${key}-${editorVersion}`} value={editorValues[key] ?? ''} label={label} projectId={projectId} onProblem={setError} autoFocus={!focusTitle && index === 0}
