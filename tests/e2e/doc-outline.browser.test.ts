...
test('in-page outline navigates H1/H2 on desktop and mobile and follows editor changes', async () => {
  ...
  try {
    ...
    const done = page.getByRole('button', { name: 'Done editing', exact: true });
    const doneBefore = await done.boundingBox();
    const readingBody = page.locator('.doc-reading-body');
    await readingBody.evaluate(el => { el.scrollTop = el.scrollHeight; });
    assert.deepEqual(await done.boundingBox(), doneBefore);
    await expect(done).toBeInViewport();
    await page.screenshot({ path: 'data/doc-done-desktop.png' });
    await page.setViewportSize({ width: 390, height: 844 });
    const mobileDoneBefore = await done.boundingBox();
    await readingBody.evaluate(el => { el.scrollTop = 0; });
    await readingBody.evaluate(el => { el.scrollTop = el.scrollHeight; });
    assert.deepEqual(await done.boundingBox(), mobileDoneBefore);
    await expect(done).toBeInViewport();
    const actionsBox = await page.getByRole('group', { name: 'Editing actions', exact: true }).boundingBox();
    const contentBox = await readingBody.boundingBox();
    assert.ok(actionsBox && contentBox && actionsBox.y + actionsBox.height <= contentBox.y);
    assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth));
    await page.screenshot({ path: 'data/doc-done-mobile.png' });
    await page.setViewportSize({ width: 1440, height: 1000 });
    ...
    // A pinned action must bring failed-save feedback into view from far below.
    const saveRoute = `**/api/doc-pages/${record.id}`;
    await page.route(saveRoute, route => route.request().postDataJSON()?.finalizeAssets ? route.abort() : route.continue());
    await readingBody.evaluate(el => { el.scrollTop = el.scrollHeight; });
    await done.click();
    await expect(page.locator('.page-editor').getByRole('alert')).toBeInViewport();
    await expect(page.locator('.page-editor').getByRole('alert')).toBeFocused();
